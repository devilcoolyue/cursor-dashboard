...
淡入淡出的第二个条件是 `.card:has(:focus-visible)`，**不要写回 `:focus-within`**：弹窗关掉时
`PanelUI` 会把焦点还给当初点开它的那颗按钮（无障碍要求，别去掉），`:focus-within` 认这个焦点，
于是鼠标点一次「调整分组 / 重新授权」，回来后那张卡的按钮就永久亮着、周期环再也不出现，
鼠标移出卡片也不消失。`:focus-visible` 只在键盘操作时成立，鼠标点开的那次不会粘住。
