    ...
    <SettingsSection title="软件更新" :description="isDesktop ? '下载完成后自动安装并重启，保留本地账号和设置。' : '检查正式发布的最新版本。服务器升级由实例管理员执行。'">
      ...
      <div v-if="release" class="about-release" aria-live="polite">
        ...
        <template v-if="release.available">
          ...
          <details v-if="release.notes" class="about-notes"><summary>本次更新内容</summary><p>{{ release.notes }}</p></details>
        </template>
      </div>
...
.about-notes summary { cursor: pointer; }
.about-notes p { white-space: pre-wrap; overflow-wrap: anywhere; line-height: 1.8; max-height: 320px; overflow-y: auto; }
