...
onMounted(() => { if (!isDesktop) void run(load) })
...
<template><section class="workspace-page settings-page"><header class="page-heading"><div><p class="eyebrow">{{ isDesktop ? '本地用户' : me?.login }}</p><h1>个人设置</h1></div></header><p v-if="error" role="alert" class="error">{{ error }}</p>
  ...
  <DesktopSettings v-if="isDesktop" /><section v-if="!isDesktop" class="settings-section"><h2>更改密码</h2><form class="form-stack narrow" @submit.prevent="changePassword"><label>当前密码<input v-model="oldPassword" type="password" autocomplete="current-password" required maxlength="256" /></label><label>新密码<input v-model="newPassword" type="password" autocomplete="new-password" required minlength="12" maxlength="256" /></label><p class="muted">至少 12 个字符。修改后包括本次登录在内的全部会话将撤销。</p><button class="primary" :disabled="busy">更新密码并退出登录</button></form></section>
  <section v-if="!isDesktop" class="settings-section"><div class="section-heading"><h2>登录会话</h2><button :disabled="busy" @click="run(load)">重载会话</button></div><div v-for="session in sessions" :key="session.id" class="setting-row"><div><strong>{{ session.current ? '当前会话' : '其他会话' }}</strong><small>登录 {{ timeText(session.created_at) }} · 到期 {{ timeText(session.expires_at) }}</small></div><button :disabled="busy" @click="revoke(session)">{{ session.current ? '退出本次登录' : '撤销会话' }}</button></div></section>
</section></template>
