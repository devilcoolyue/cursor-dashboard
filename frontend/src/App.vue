...
import { me, activeSpace, workspaceId, selectSpace, clearIdentity, initialize, loadMe, ready, startupError } from './state'
...
watch(me, value => { if (!value) { creating.value = false; teamName.value = ''; error.value = ''; if (!route.meta.public) void router.replace('/login') } })
...
<template>
  <div v-if="!ready" class="auth-page" role="status">正在连接服务…</div>
    ...
    <div v-if="me && !route.meta.public" class="app-shell">
      ...
      <aside class="sidebar" :class="{ expanded: menuOpen }"><RouterLink class="wordmark" to="/accounts">CURSOR PANEL<span class="brand-dot">.</span></RouterLink><button class="mobile-toggle" :aria-expanded="menuOpen" aria-label="展开导航" @click="menuOpen = !menuOpen">☰</button>
        <div class="sidebar-content"><label class="space-selector"><span>当前空间</span><select :value="workspaceId" @change="changeSpace"><option v-for="space in me.workspaces" :key="space.id" :value="space.id">{{ space.kind === 'personal' ? '个人 · ' : '团队 · ' }}{{ space.name }}</option></select></label>
          ...
          <details class="team-options"><summary>团队协作</summary><button @click="creating = true; error = ''">＋ 创建团队空间</button><RouterLink to="/join">接受团队邀请</RouterLink></details>
          <div class="sidebar-bottom"><span class="user-avatar">{{ me.login.slice(0, 1).toUpperCase() }}</span><div><strong>{{ me.login }}</strong><button :disabled="busy" @click="logout">退出登录</button></div></div>
        </div>
      </aside>
      <main id="main-content" class="main-content" tabindex="-1"><div class="topbar"><span>{{ activeSpace?.name }}</span><span class="muted">Cursor · Web</span></div><p v-if="error && !creating" role="alert" class="error global-error">{{ error }}</p><RouterView :key="`${me.id}:${workspaceId}:${route.path}`" /></main>
    </div>
    <RouterView v-else />
  </template>
