...
import { me, activeSpace, workspaceId, selectSpace, clearIdentity, initialize, loadMe, ready, startupError, desktopStatus, activeConnection, bootstrap } from './state'
...
watch(me, value => { if (value && isDesktop && route.meta.public && route.path !== '/connections') void router.replace('/accounts'); if (!value) { creating.value = false; teamName.value = ''; error.value = ''; if (!route.meta.public) void router.replace(isDesktop ? '/connections' : '/login') } })
...
<template>
  ...
  <template v-else>
    <div v-if="me && (!route.meta.public || route.path === '/connections')" class="app-shell">
      ...
      <aside class="sidebar" :class="{ expanded: menuOpen }"><RouterLink class="wordmark" to="/accounts">CURSOR PANEL<span class="brand-dot">.</span></RouterLink><button class="mobile-toggle" :aria-expanded="menuOpen" aria-label="展开导航" @click="menuOpen = !menuOpen">☰</button>
        <div class="sidebar-content"><RouterLink v-if="isDesktop" to="/connections" class="connection-selector">{{ activeConnection?.name || '本地' }} · 切换实例</RouterLink><label class="space-selector"><span>当前空间</span><select :value="workspaceId" @change="changeSpace"><option v-for="space in me.workspaces" :key="space.id" :value="space.id">{{ space.kind === 'personal' ? '个人 · ' : '团队 · ' }}{{ space.name }}</option></select></label>
          ...
          <div class="sidebar-bottom"><span class="user-avatar">{{ me.login.slice(0, 1).toUpperCase() }}</span><div><strong>{{ isDesktop && !connectionId ? '本地用户' : me.login }}</strong><span v-if="isDesktop" class="muted">{{ activeConnection?.name || '本机个人空间' }}</span><button v-else :disabled="busy" @click="logout">退出登录</button></div></div>
        </div>
      </aside>
      <main id="main-content" class="main-content" tabindex="-1"><div class="topbar"><span>{{ activeSpace?.name }}</span><span class="muted">Cursor · {{ isDesktop ? 'Desktop' : 'Web' }}</span></div><p v-if="error && !creating" role="alert" class="error global-error">{{ error }}</p><DesktopStatusBar v-if="isDesktop" /><p v-if="connectionId && bootstrap && !bootstrap.capabilities.remote_switch && route.path === '/accounts'" class="notice">此实例尚未开放远程切换。你可以查看额度并执行获授权的账号操作。</p><RouterView :key="`${connectionId || 'local'}:${me.id}:${workspaceId}:${route.path}`" /></main>
    </div>
    <RouterView v-else />
  </template>
