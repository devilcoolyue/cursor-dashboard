...
<template><UiDialog :title="restoreId ? '恢复 Cursor 备份' : terminal ? '在本机终端切换' : '切换本机 Cursor'" :wide="terminal" @close="emit('close')">
  <p v-if="account"><strong>{{ account.label }}</strong> · {{ account.email }}</p>
  <template v-if="terminal && account">
    <SwitchCommandForm :account="account" local :initial-platform="detection?.platform" />
    <div class="actions"><button @click="terminal = false">返回直接切换</button></div>
  </template>
  <template v-else>
  ...
  <div v-if="account && !restoreId && !connectionId && !started && !state?.busy" class="actions"><button :disabled="busy || !['macos', 'windows'].includes(detection?.platform || '')" @click="terminal = true">终端执行</button></div>
