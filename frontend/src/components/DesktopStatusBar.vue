...
import { native, type DesktopStatus } from '../platform'
import { desktopStatus } from '../state'
const failed = ref(false)
let timer: ReturnType<typeof setTimeout> | undefined, alive = true
async function poll() {
  try { const state = await native<DesktopStatus>('status'); if (alive) { desktopStatus.value = state; failed.value = false } }
