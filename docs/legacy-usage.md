...
旧版归档基线的包版本为 `1.4.0`（`legacy` 分支）。文档按 2026-09-07 的代码整理，核查范围、验证结果和已知限制见[阶段归档](archive/2026-09-07.md)。数据来自 Cursor 非公开接口，不能视为官方计费或兼容性承诺。
...
服务器部署、环境变量、数据库迁移和备份见[部署与运行](operations.md)。对外监听前设置 `PANEL_TOKEN`，并配置 HTTPS；程序缺少口令时只警告，不会拒绝启动。
