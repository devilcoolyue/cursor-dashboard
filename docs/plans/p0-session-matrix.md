...
2026-09-10 P5 更新：设备身份、S07 撤权竞争和 S08 本地执行/恢复已使用合成账号验证，见 [P5 报告](p5-verification.md)。`tests/test_v2_connected.py` 新增两个严格单次 RT 反例模型，证明自身数据库锁不能协调外部客户端；这些模型不是 Cursor 实际接口结论。S01–S06 仍未执行，生产远程切换关闭。
