...
- 页面先从 `/api/account-index` 读取当前部门的安全索引，再按账号渐进加载额度；完成一张
  就显示一张，并实时更新“已加载 N / 总数”。切换部门会取消上一轮未完成请求。
- 浏览器记住最后选择的部门；刷新页面或点击“刷新当前”只请求该部门。只有选择“全部”
  时才读取全部账号。
- 账号默认按添加顺序显示，也可按综合剩余百分比从低到高或从高到低排序；浏览器会记住
  最后选择的排序方式。
