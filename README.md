...
| `MAX_WORKERS` | 48 | 取数线程池。全是网络等待，调大不占 CPU |
...
取数是两层并发：**账号之间**并发，**单账号内的 5 个接口**也并发
（`server.probe` 用 `asyncio.gather` 打平成 N×5 个任务，共用一个线程池）。

实测（单账号 5 个接口，走代理访问 cursor.com）：

| 场景 | 耗时 |
|---|---|
| 1 账号回源 | 0.83s（改造前串行 2.0s） |
| 4 账号回源 | 0.89s（和 1 个账号基本持平） |
| 命中缓存 | 0.0015s |
| 5 个访客同时强制刷新 4 账号 | 1.10s（对照：串行 5 次 5.81s） |
