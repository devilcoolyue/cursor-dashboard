...
- **Updates require published update packages.** Source includes update checks and signed update support. Signed update packages are included in v0.0.4; server upgrades also need the optional Linux amd64 host updater. Users of v0.0.1 must install v0.0.2 manually once to get the updater. See [automatic updates](docs/automatic-updates.md).
